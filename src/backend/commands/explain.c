 ...
 */
static void
ExplainQueryGraph (Query *query, ExplainStmt *stmt,
		  const char *queryString, ParamListInfo params,
		  TupOutputState *tstate)
{
	StringInfoData buf;

	/* generate Query graph dot commands */
	initStringInfo(&buf);

	appendStringInfo(&buf, "%s", (dotQuery(query))->data);
...
/*
 * Extends the "normal" explain for provenance queries. In addition to the query plan the list of provenance rewrite strategies used
 * to produce the provenance query.
 */
			   ...
			   ExplainStmt *stmt, TupOutputState *tstate)
{
	...
	int			eflags;
		...
		if (query && query->provInfo)
		{
			...
			info = (ProvInfo *) query->provInfo;
			methods = (List *) info->rewriteInfo;
			...
			foreach(lc, methods)
			{
				...
				if (lc->next != NULL)
					appendStringInfo(&buf, "%s,",val->val.str);
				else
					appendStringInfo(&buf, "%s",val->val.str);
			}
