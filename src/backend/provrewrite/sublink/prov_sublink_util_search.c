...
static bool findSublevelsUpVarWalker (Node *node, GetCorrelatedVarsWalkerContext *context);
static bool findCorrelatedVarsQueryTreeWalker (Query *query, GetCorrelatedVarsWalkerContext *context);
static bool findExprVarWalker (Node *node, List **context);
static SublinkInfo *makeSublinkInfo (SubLink *sublink, Node *rootExpr, Node **sublinkExprPointer, Node **grandParentRef, Node *parent);
static bool findLinkToExprWalker (Node *node, FindLinkToExprWalkerContext *context);
...
List *getSublinkBaseRelations(Query *query)
{
        List *result;
...
/*
 * expression walker used to find sublinks. All sublinks that are found are appended to the context list.
 */
        ...
        if (IsA(node, SubLink))
        {
                ...
                *context = lappend(*context, sublink);
                ...
                if (sublink->testexpr != NULL)
                        return expression_tree_walker((Node *) sublink->testexpr, findExprSublinkWalker, (void *) context);
...
/*
 * Searches a query node for sublinks (in targetList, joinExpr, groupClause) and returns a list of sublinkinfos for
 * these sublinks. The flags define which possible locations should be considered. (See prov_sublink_util.h)
 */
...
findSublinkLocations (Query *query, int flags)
{
        ...
        if (flags & PROV_SUBLINK_SEARCH_WHERE)
        {
                ...
                foreach(lc, query->jointree->fromlist)
                {
                        ...
                        {
                                result = list_concat(result, extractJoinExprSublinks(((JoinExpr *) node)));
                        }
                }
        }
...
extractJoinExprSublinks (JoinExpr *joinExpr)
{
        ...
        {
                result = list_concat(result, extractJoinExprSublinks((JoinExpr *) joinExpr->larg));
        }
        if (IsA(joinExpr->rarg, JoinExpr))
        {
                result = list_concat(result, extractJoinExprSublinks((JoinExpr *) joinExpr->rarg));
        }
        return result;
}

/*
 * Returns a trimmed list of sublink infos that contains only sublinks with the category indicated by the flags.
 */
...
findVarsSublevelsUp (Query *query, SublinkInfo *info)
{
        ...
        context = (GetCorrelatedVarsWalkerContext *) palloc(sizeof(GetCorrelatedVarsWalkerContext));
...
/*
 * Walks an expression tree and builds a list of the var nodes used in
 * the expression.
 */
...
findSublevelsUpVarWalker (Node *node, GetCorrelatedVarsWalkerContext *context)
{
        ...
        newContext = (GetCorrelatedVarsWalkerContext *) palloc(sizeof(GetCorrelatedVarsWalkerContext));
        ...
        if (IsA(node, Var))
        {
                ...
                {
                        vars = NIL;//TODO
                        corrVarInfo = makeCorrVar (var, vars, context->parent, context->exprRoot, context->location,
                                        var->varlevelsup >= context->varlevelsUp, NULL);
                        *(newContext->correlated) = lappend(*(newContext->correlated), corrVarInfo);
                }
        ...
        else if (IsA(node,Query))
        {
                ...
                return findCorrelatedVarsQueryTreeWalker((Query *) node, newContext);
        }

        // recurse
        return expression_tree_walker(node, findSublevelsUpVarWalker, (void *) context);
}
...
static bool
findCorrelatedVarsQueryTreeWalker (Query *query, GetCorrelatedVarsWalkerContext *context)
{
        ListCell *lc;
        Node *node;
        List *groupByTes;
        ...
        /* walk range table */
        if (range_table_walker(query->rtable, findSublevelsUpVarWalker, (void *) context, QTW_IGNORE_JOINALIASES))
                return true;
...
findSublinksForExpr (Node *expr)
{
        ...
        foreach(lc, sublinks)
        {
                ...
                newInfo = extractSublinks(copyExpr,copyExpr, &copyExpr2, NULL, sublink, NULL);
                newInfo2 = extractSublinks(copyExpr2, copyExpr2, &copyExpr2, NULL, sublink, NULL);
...
/*
 * Searches for Sublinks inside an expression and returns a list of SublinkInfos for all Sublinks found.
 */

SublinkInfo *
extractSublinks (Node *node, Node *exprRoot, Node **parentRef, Node **grandParentRef, SubLink* sublink, Node *parent)
{
        ListCell *lc;
        SublinkInfo *info;
        ...
        switch (node->type)
        {
                case T_SubLink:
                {
                        ...
                        {
                                info = makeSublinkInfo(foundSublink, exprRoot, parentRef, grandParentRef, parent);
                                return info;
                        }

                        info = extractSublinks(foundSublink->testexpr, exprRoot, &(foundSublink->testexpr), parentRef, sublink, node);
                ...
                case T_A_Expr:
                {
                        ...
                        info = extractSublinks(aExpr->lexpr, exprRoot, &(aExpr->lexpr), parentRef, sublink, node);

                        if (info)
                                return info;

                        info = extractSublinks(aExpr->rexpr, exprRoot, &(aExpr->rexpr), parentRef, sublink, node);
                ...
                case T_TypeCast:
                {
                        ...
                        return extractSublinks(typeCast->arg, exprRoot, &(typeCast->arg), parentRef, sublink, node);
                }
                break;
                case T_A_Indirection:
                {
                        ...
                        return extractSublinks(aInd->arg, exprRoot,  &(aInd->arg), parentRef, sublink, node);
                }
                break;
                case T_RowExpr:
                {
                        ...
                        {
                                info = extractSublinks((Node *) lfirst(lc), exprRoot,  (Node **) &(lc->data.ptr_value), parentRef, sublink, node);
                                if (info)
                                        return info;
                        }
                }
                ...
                case T_XmlSerialize:
                {
                        ...
                        return extractSublinks(xmlSer->expr, exprRoot,  &(xmlSer->expr), parentRef, sublink, node);
                }
                break;
                case T_NullTest:
                {
                        ...
                        return extractSublinks((Node *) nullTest->arg, exprRoot,   (Node **) &(nullTest->arg), parentRef, sublink, node);
                }
                break;
                case T_BooleanTest:
                {
                        ...
                        return extractSublinks((Node *) boolTest->arg, exprRoot,  (Node **) &(boolTest->arg), parentRef, sublink, node);
                }
                break;
                case T_Aggref:
                {
                ...
                case T_CaseWhen:
                {
                        ...
                        info = extractSublinks((Node *) caseWhen->expr, exprRoot,   (Node **) &(caseWhen->expr), parentRef, sublink, node);

                        if (info)
                                return info;

                        info = extractSublinks((Node *) caseWhen->result, exprRoot,   (Node **) &(caseWhen->result), parentRef, sublink, node);
                ...
                case T_FieldSelect:
                {
                        ...
                        return extractSublinks((Node *) fieldSel->arg, exprRoot,  (Node **) &(fieldSel->arg), parentRef, sublink, node);
                }
                break;
                ...
                case T_RelabelType:
                {
                        ...
                        return extractSublinks((Node *) relab->arg, exprRoot,  (Node **) &(relab->arg), parentRef, sublink, node);
                }
                break;
                case T_CoerceViaIO:
                {
                        ...
                        return extractSublinks((Node *) co->arg, exprRoot,   (Node **) &(co->arg), parentRef, sublink, node);
                }
                break;
                case T_ArrayCoerceExpr:
                {
                        ...
                        return extractSublinks((Node *) acExpr->arg, exprRoot,   (Node **) &(acExpr->arg), parentRef, sublink, node);
                }
                break;
                case T_ConvertRowtypeExpr:
                {
                        ...
                        return extractSublinks((Node *) conRowExpr->arg, exprRoot,   (Node **) &(conRowExpr->arg), parentRef, sublink, node);
                }
                break;
                case T_CoerceToDomain:
                {
                        ...
                        return extractSublinks((Node *) coerceDom->arg, exprRoot,   (Node **) &(coerceDom->arg), parentRef, sublink, node);
                }
                break;
                case T_ArrayExpr:
                {
...
static SublinkInfo *
makeSublinkInfo (SubLink *sublink, Node *rootExpr, Node **sublinkExprPointer, Node **grandParentRef, Node *parent)
{
        SublinkInfo *info;
        List *outsideVars;
        ...
        {
                outsideVars = findCorrVarTypes(info->corrVarInfos, PROV_CORRVAR_OUTSIDE);
...
findRTRefInFromExpr (Node *fromItem, Index rtIndex)
{
        ...
        else
        {
                ...
                return (findRTRefInFromExpr (joinExpr->larg, rtIndex) || findRTRefInFromExpr (joinExpr->rarg, rtIndex));
        }
}
...
findAccessedBaseRelations (Query *query)
{
        ...
        foreach(lc, query->rtable)
        {
                ...
                if (rte->rtekind == RTE_RELATION)
                {
                        ...
                        if (strcmp("*NEW*", rte->eref->aliasname) != 0 && strcmp("*OLD*", rte->eref->aliasname) != 0)
                        {
                ...
                {
                        result = list_concat(result, findAccessedBaseRelations(rte->subquery));
                }
        }
...
/*
 * Searches through a list of sublink infos and returns all sublinks that are at one of the locations (SELECT, WHERE, ...)
...
findLinkToExpr (Node *expr, Node *root)
{
        ...
        context = (FindLinkToExprWalkerContext *) palloc(sizeof(FindLinkToExprWalkerContext));
        context->expr = expr;
        context->parent = &root;
        context->result = NULL;
...
/*
 * Checks if a sublink s contains correlated sublinks that reference an relation outside s that will
 * be rewritten using the GEN-strategy. This means that s cannot be unnested because it would be
 * used in the FROM-clause and no correlations with other FROM-clause items are allowed there.
 */
...
containsGenCorrSublink (SublinkInfo *info)//TODO remove
{
        ...
        /* otherwise check is contained sublinks have correlations referencing attributes outside
         *  the sublink and will be rewritten using GEN-strategy.
         */
        foreach(lc, subInfos)
        {
                ...
                {
                        outCorrVars = findCorrVarTypes(subInfo->corrVarInfos, PROV_CORRVAR_OUTSIDE);
