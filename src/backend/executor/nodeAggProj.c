...
/*
 * ExecAggProj for non-hashed case
 */
static TupleTableSlot *
aggProj_retrieve_direct(AggProjState *aggstate)
{
	Agg		   *node = (Agg *) aggstate->ss.ps.plan;
	PlanState  *outerPlan;
...
//	TupleTableSlot *tempSlot;
	...
	tuplestorestate = aggstate->tuplestorestate;
	 ...
	 * aggstate->ss.ps.qual
	 */
	while (!aggstate->agg_done && !aggstate->output)
	{
		/*
		...
		if (aggstate->grp_firstTuple != NULL)
		{
			...
			if (tuplestorestate == NULL)
			{
				...
				aggstate->tuplestorestate = tuplestorestate;
			}
			...
			for (;;)
			{
				...
				if (node->aggstrategy == AGG_SORTED)
				{
										 ...
										 tmpcontext->ecxt_per_tuple_memory))
					{
						 ...
						 */
						aggstate->grp_firstTuple = ExecCopySlotTuple(outerslot);
						// put into tuple store
						//tuplestore_puttupleslot(tuplestorestate, outerslot);
						aggstate->tempTupleTableSlot = outerslot;
						aggstate->newGroup = TRUE;
						break;
					}
					else
		...
		if (ExecQual(aggstate->ss.ps.qual, econtext, false))
		{
			...
			//REMOVE and add code to update your group/count info
			aggstate->output = TRUE;
			break;
		}
		else
		{
			// TODO release tuple store
		}
	}
	...
	if (aggstate->output)
	{
		if (aggstate->count > 0)
		{
			...
			if (tuplestorestate != NULL && !tuplestore_ateof(tuplestorestate))
			{
				...
				if (tuplestore_gettupleslot(tuplestorestate, 1, firstSlot))
				{
					//ResetExprContext(econtext);
					ExecCopySlot(econtext->ecxt_outertuple, firstSlot);
				}
				else
					ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION), errmsg("%s", "get tuple failed")));
			}
			else
				ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION), errmsg("%s", "tuple store uninitialized or tuple store reached the end.")));
 ...
 * ExecInitAggProj
 *
 *	Creates the run-time information for the aggProj node produced by the
 *	planner and initializes its outer subtree
 * -----------------
 */
AggProjState *
ExecInitAggProj(AggProj *node, EState *estate, int eflags)
{
	AggProjState   *aggstate;
	AggStatePerAgg peragg;
	Plan	   *outerPlan;
	ExprContext *econtext;
	int			numaggs,
	 ...
	 */
	aggstate = makeNode(AggProjState);
	...
	// initialize output mode: 0 - group create, 1 - group output
	aggstate->output = FALSE;
	aggstate->count = 0;
	aggstate->newGroup = FALSE;
...
#define AGG_NSLOTS 3
	...
	//TODO create TUPLESTORE AND evaluations expressions for the projections in the aggproj clause
...
int
ExecCountSlotsAggProj(AggProj *node)
{
...
void
ExecReScanAggProj(AggProjState *node, ExprContext *exprCtxt)
{
