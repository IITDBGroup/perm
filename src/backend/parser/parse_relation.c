				 ...
				 int location)
{
	...
	//TODO add column erefs for provenance attrs here
			 ...
			 int location)
{
	...
	while (pstate != NULL)
	{
		...
		foreach(l, pstate->p_varnamespace)
		{
			...
			{
				if (result) {
					//CHANGED for provenance attrs we ignore ambiguous colnames and check later in provrewrite
					//TODO not correct anymore
					//if (strncmp(colname,"prov_",5) != 0)
						ereport(ERROR,
								(errcode(ERRCODE_AMBIGUOUS_COLUMN),
								 errmsg("column reference \"%s\" is ambiguous",
										colname),
...
expandTransProvenanceRTE (RangeTblEntry *rte, List **colnames, List **colvars, Index rtindex)
{
	...
	newName = makeString(pstrdup("trans_prov"));

	/* each transformation provenance type produces one extra attribute, but
	 * the data type produced are different.
	 */
	switch(ContributionType(rte->subquery))
	{
	...
	case CONTR_TRANS_XML:
		newVar = makeVar(rtindex, list_length(*colvars) + 1, XMLOID, -1, 0);
		break;
	default:
		//TODO error
		break;
 ...
 * have to walk throuh the leaf base relation RTEs accessed by the query to create the
 *  list of provenance attributes with correct names and ordering.
 */

static void
expandProvenanceRTE (RangeTblEntry *rte, List **colnames, List **colvars, Index rtindex)
{
	ListCell *lc;
	ListCell *colLc;
	ListCell *varLc;
	...
	/* find leaf RTEs representing base relations */
	baseRelRTEs = NIL;
	findBaseRelationsForProvenanceRTE (rte, &baseRelRTEs);

	/* reset reference counters for base relations */
	resetRelReferences();

	/* add columnnames and vars for reach base relation RTE */
	if (rte->eref)
		curAttno = list_length(rte->eref->colnames);
	else
		curAttno = 0;

	foreach(lc, baseRelRTEs)
	{
		curRTE = (RangeTblEntry *) lfirst(lc);

		/* increment rel ref counter */
		getQueryRefNum(curRTE, true);

		/* get vars and columnnames for rte */
		curCols = NIL;
		curVars = NIL;

		if (curRTE->rtekind == RTE_RELATION)
			expandRelation(curRTE->relid, curRTE->eref, 1, 0, false, &curCols, &curVars);
		else
			expandRTEWithParam (curRTE, rtindex, 0, false, false, &curCols, &curVars);

		forboth(colLc, curCols, varLc, curVars)
		{
			curAttno++;

			if (colnames)
			{
				curCol = (Value *) lfirst(colLc);
				curCol->val.str = createProvAttrName (curRTE, curCol->val.str);

				*colnames = lappend(*colnames, curCol);
			}

			if (colvars)
			{
				curVar = (Var *) lfirst(varLc);

				curVar->varattno = curAttno;
				curVar->varno = rtindex;

				*colvars = lappend(*colvars, curVar);
			}
		}
	}
}
...
/*
 * a variant of expandRTE with the possiblity to omit adding provenance attributes.
 * This is used by the provenance module.
 */

void
expandRTEWithParam (RangeTblEntry *rte, int rtindex, int sublevels_up,
		  bool include_dropped, bool addProvenanceAttrs,
		  List **colnames, List **colvars)
{
	int			varattno;
		...
		case RTE_SUBQUERY:
			{
				...
				//TODO add erefand tes here colnames here if provenance query
				//CHECK do we need to add TEs for the subquery too? are subqueries set before here???
				/*
				 * If subquery is marked for provenance rewrite we have to add aliases for
				 * provenance columns.
				 */
				if (IsProvRewrite(rte->subquery) && addProvenanceAttrs)
				{
					switch(ContributionType(rte->subquery))
					{
						...
						case CONTR_INFLUENCE:
							expandProvenanceRTE(rte, colnames, colvars, rtindex);
						break;
						case CONTR_TRANS_SET:
						case CONTR_TRANS_SQL:
						case CONTR_TRANS_XML:
...
/*
 * Add provenance attributes to the target list of a query that is marked for
 * provenance rewrite.
 */

void
addProvenanceTEs (RangeTblEntry *rte) {
	...
	curResno = 1;
	/* check if provenance attributes have been added to targetlist before */
	...
	/* create vars for provenance attrs */
	switch(ContributionType(rte->subquery))
	{
		case CONTR_COPY_COMPLETE_TRANSITIVE:
		case CONTR_COPY_COMPLETE_NONTRANSITIVE:
		...
		case CONTR_COPY_PARTIAL_NONTRANSITIVE:
		case CONTR_INFLUENCE:
			expandProvenanceRTE(rte, &names, &provVars, 1);
		break;
		case CONTR_TRANS_SET:
		case CONTR_TRANS_SQL:
		case CONTR_TRANS_XML:
	...
	/* adapt rte eref */
	rte->eref->colnames = list_concat(rte->eref->colnames, names);

	/* create target entries and append them to targetlist */
	forboth(varLc, provVars, lc, names)
	{
		var = (Var *) lfirst(varLc);
		name = pstrdup(((Value *) lfirst(lc))->val.str);

		te = makeTargetEntry((Expr *) var, curResno, name, false);
		rte->subquery->targetList = lappend(rte->subquery->targetList, te);
