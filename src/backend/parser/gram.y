...
%type <node>	table_ref
%type <boolean> from_item_provenance_baserel_clause
%type <list>	from_item_with_provenance_clause
...
%type <node> opt_provenance
%type <list> opt_annot from_item_annot_clause
%type <node> xslt_func_expr
%type <node> this_expr
%type <list> annot_list
%type <ival> contribution_type
%type <boolean> opt_copy_contribution_trans opt_copy_contribution_mod
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	BACKWARD BASERELATION BEFORE BEGIN_P BETWEEN BIGINT BINARY BIT
	BOOLEAN_P BOTH BY

	CACHE CALLED CASCADE CASCADED CASE CAST CHAIN CHAR_P
	CHARACTER CHARACTERISTICS CHECK CHECKPOINT CLASS CLOSE
	CLUSTER COALESCE COLLATE COLUMN COMMENT COMMIT
	COMMITTED COMPLETE CONCURRENTLY CONFIGURATION CONNECTION CONSTRAINT CONSTRAINTS
	CONTENT_P CONTRIBUTION CONVERSION_P COPY COST CREATE CREATEDB
	...
	MAPPING MAPPROV MATCH MAXVALUE MINUTE_P MINVALUE MODE MONTH_P MOVE

	NAME_P NAMES NATIONAL NATURAL NCHAR NEW NEXT NO NOCREATEDB
	NOCREATEROLE NOCREATEUSER NOINHERIT NOLOGIN_P NONE NOSUPERUSER
	NOT NOTHING NOTIFY NOTNULL NOTTRANSITIVE NOWAIT NULL_P NULLIF NULLS_P NUMERIC
	...
	PRIOR PRIVILEGES PROCEDURAL PROCEDURE PROVENANCE
	...
	TABLE TABLESPACE TEMP TEMPLATE TEMPORARY TEXT_P THEN THIS TIME TIMESTAMP
	TO TRAILING TRANSACTION TRANSITIVE TRANSPROV TRANSSQL TRANSXML TREAT TRIGGER TRIM TRUE_P
			...
			group_clause having_clause
				{
					...
					((ProvInfo *) n->provenanceClause)->annotations = $2;
					n->distinctClause = $4;
					n->targetList = $5;
					n->intoClause = $6;
					n->fromClause = $7;
...
opt_provenance:
			PROVENANCE								
				{	
					...
					n->shouldRewrite = true;
					n->contribution = CONTR_INFLUENCE;
					$$ = (Node *) n; 
				}
			| PROVENANCE ON CONTRIBUTION '(' contribution_type ')'								
				{	
					ProvInfo *n = makeProvInfo();
					n->shouldRewrite = true;
					n->contribution = $5;
			...
			| TRANSPROV
				{
					ProvInfo *n = makeProvInfo();
					n->shouldRewrite = true;
					n->contribution = CONTR_TRANS_SET;
					$$ = (Node *) n;
				}
			| TRANSSQL
				{
					ProvInfo *n = makeProvInfo();
					n->shouldRewrite = true;
					n->contribution = CONTR_TRANS_SQL;
					$$ = (Node *) n;
				}
			| TRANSXML
				{
					ProvInfo *n = makeProvInfo();
					n->shouldRewrite = true;
					n->contribution = CONTR_TRANS_XML;
					$$ = (Node *) n;
				}
			| TRANSXML ON
				{
					ProvInfo *n = makeProvInfo();
					n->shouldRewrite = true;
					n->contribution = CONTR_TRANS_XML_SIMPLE;
					$$ = (Node *) n;
				}
			| MAPPROV
				{
...
contribution_type:
			COPY opt_copy_contribution_mod opt_copy_contribution_trans
				{
					if ($2 && $3) 
					{
 ...
 */
table_ref:	relation_expr from_item_annot_clause from_item_provenance_baserel_clause from_item_with_provenance_clause
				{
					$1->isProvBase = $3;
					$1->provAttrs = $4;
					$1->annotations = $2;
			...
			| relation_expr from_item_annot_clause from_item_provenance_baserel_clause from_item_with_provenance_clause alias_clause
				{
					$1->alias = $5;
					$1->isProvBase = $3;
					$1->provAttrs = $4;
			...
			| select_with_parens from_item_annot_clause from_item_provenance_baserel_clause from_item_with_provenance_clause alias_clause
				{
...
from_item_provenance_baserel_clause:
			BASERELATION							{ $$ = true; }
			| /* EMPTY */							{ $$ = false; }
		;

from_item_with_provenance_clause:
			PROVENANCE '(' name_list ')'			{ $$ = $3; }
			| /* EMPTY */							{ $$ = NIL; }
		;
		
from_item_annot_clause:
			ANNOT '(' annot_list ')'				{ $$ = (List *) $3; }
			| /* EMPTY */							{ $$ = NIL; }
...
/*
 * xslt expression is a short cut for calling a XSLT function that
 * is stored in table xslt_funcs. 
 */

xslt_func_expr:	XSLT '.'  type_function_name '(' a_expr ')'
				{
					XsltFuncExpr *x = makeNode(XsltFuncExpr);
					x->funcName = $3;
					x->param = $5;
					$$ = (Node *)x;
				}
