...
transformSelectStmt(ParseState *pstate, SelectStmt *stmt)
{
	...
	/* copy the provenance rewrite hint */
	qry->provInfo = (Node *) stmt->provenanceClause;
	SetSublinkRewritten(qry,false);
			   ...
			   List **colTypes, List **colTypmods)
{
	...
	*colTypmods = NIL;
	if (IsA(node, RangeTblRef))
	{
		...
		/* is query is a provenance query we have to add target entries for provenance attributes */
		if (IsProvRewrite(selectQuery))
		{
