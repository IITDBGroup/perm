...
bool prov_use_sublink_move_to_target = true;
...
static struct config_bool ConfigureNamesBool[] =
{
	...
	{
		{"prov_use_set_optimization", PGC_USERSET, QUERY_TUNING,
			gettext_noop("Use the optimized form of set operator provenance rewriting"),
			gettext_noop("Deactivating causes each set operation in a set operation"
						 "tree to be outsourced into a separate query node."),
		},
		&prov_use_set_optimization,
		true, NULL, NULL
	},

	{
		{"prov_use_sublink_optimization_left_join", PGC_USERSET, QUERY_TUNING,
			gettext_noop("Use the left join rewrite method for sublinks"),
			gettext_noop("If actived the left join rewrite method will be used"
						 "for uncorrelated sublinks."),
		},
		&prov_use_sublink_optimization_left_join,
		true, NULL, NULL
	},

	{
		{"prov_use_sublink_move_to_target", PGC_USERSET, QUERY_TUNING,
			gettext_noop("Move sublinks to target list."),
			gettext_noop("If actived sublinks are moved to the target list of a query."
	...
	{
		{"prov_use_sublink_transfrom_top_level_any_to_join", PGC_USERSET, QUERY_TUNING,
			gettext_noop("If an any sublink is an top level sublink transform it into a join."),
			gettext_noop("If actived any sublinks that are top level are transformed "
						 "into a join on the sublink condition."),
		},
		&prov_use_sublink_transfrom_top_level_any_to_join,
		true, NULL, NULL
	},

	{
		{"prov_use_unnest_JA", PGC_USERSET, QUERY_TUNING,
			gettext_noop("Unnest coorelated aggregation sublinks using joins and GROUP BY."),
			gettext_noop("If actived any sublinks that are top level are transformed "
						 "into a join on the sublink condition."),
		},
		&prov_use_unnest_JA,
	...
	{
		{"prov_use_optimizer", PGC_USERSET, QUERY_TUNING,
			gettext_noop("Create query trees for all applicable rewrite methods."),
			gettext_noop("If actived Create query trees for all applicable rewrite methods are created"
						 " for a incoming query. All trees are feed into the optimizer and the"
						 " tree with the lowest estimated cost is choosen for execution."),
		},
		&prov_use_optimizer,
		false, NULL, NULL
	},
	...
	{
		{"prov_use_selection_pushdown", PGC_USERSET, QUERY_TUNING,
			gettext_noop("Try to aggressively pushdown selection above a provenance query"),
			gettext_noop("If activated the provenance rewriter module tries to push down"
						 "selections as far as possible, before applying actual rewrites. "
						 " The rationale behind this option is that provenance computation"
						 "can be very expensive and thus the number of tuples for which we"
						 "have to compute provenance should be reduced as early as possible "),
		},
		&prov_use_selection_pushdown,
		false, NULL, NULL
	},

	{
		{"prov_xml_whitespace", PGC_USERSET, CUSTOM_OPTIONS,
			gettext_noop("nicely indents xml results using "),
			gettext_noop("If activated the following functions use whitespace to pretty print"
						"XML output:"),
		},
		&prov_xml_whitespace,
		false, NULL, NULL
	...
	{
		{"prov_use_wl_union_semantics", PGC_USERSET, CUSTOM_OPTIONS,
			gettext_noop("use a different kind of semantics for union"),
			gettext_noop("If activated the the provenance of a tuple t from the "
					"result of R UNION S that is derived from a tuple r from R"
					"and a tuple s FROM S would be (t,r, null) and (t,null,s) instead"
					"of (t,r,s)"),
		},
