					   ...
					   void *context)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_SublinkInfo:
			{
				SublinkInfo *sublinkInfo = (SublinkInfo *) node;

				if (walker(sublinkInfo->sublink, context))
					return true;
			}
			break;
		case T_SelectionInfo:
			{
				SelectionInfo *sel = (SelectionInfo *) node;

				if (walker(sel->expr, context))
					return true;
				if (walker(sel->vars, context))
					return true;
			}
			break;
		case T_PushdownInfo:
			{
				PushdownInfo *push = (PushdownInfo *) node;

				if (walker(push->conjuncts, context))
					return true;
				if (walker(push->equiLists, context))
					return true;
			}
			break;
		case T_EquivalenceList:
			{
				EquivalenceList *equi = (EquivalenceList *) node;

				if (walker(equi->exprs, context))
					return true;
			}
			break;
		case T_InequalityGraph:
			{
				InequalityGraph *graph = (InequalityGraph *) node;

				if (walker(graph->equiLists, context))
					return true;
				if (walker(graph->nodes, context))
					return true;
			}
			break;
		case T_InequalityGraphNode:
			{
				InequalityGraphNode *gNode = (InequalityGraphNode *) node;

				if (walker(gNode->consts, context))
					return true;
				if (walker(gNode->equis, context))
					return true;
				if (walker(gNode->greaterEqThen, context))
					return true;
				if (walker(gNode->greaterThen, context))
					return true;
				if (walker(gNode->lessEqThen, context))
					return true;
				if (walker(gNode->lessThen, context))
					return true;
			}
			break;
		default:
...
#define MUTATE(newfield, oldfield, fieldtype)  \
	...
	switch (nodeTag(node))
	{
		...
		case T_SelectionInfo:
			{
				SelectionInfo *sel = (SelectionInfo *) node;
				SelectionInfo *newnode;

				FLATCOPY(newnode, sel, SelectionInfo);
				MUTATE(newnode->expr, sel->expr, Node *);
				MUTATE(newnode->vars, sel->vars, List *);
				return (Node *) newnode;
			}
			break;
		case T_PushdownInfo:
			{
				PushdownInfo *push = (PushdownInfo *) node;
				PushdownInfo *newnode;

				FLATCOPY(newnode, push, PushdownInfo);
				MUTATE(newnode->conjuncts, push->conjuncts, List *);
				MUTATE(newnode->equiLists, push->equiLists, List *);
				return (Node *) newnode;
			}
			break;
		case T_EquivalenceList:
			{
				EquivalenceList *equi = (EquivalenceList *) node;
				EquivalenceList *newnode;

				FLATCOPY(newnode, equi, EquivalenceList);
				MUTATE(newnode->exprs, equi->exprs, List *);
				return (Node *) newnode;
			}
			break;
		default:
