					   ...
					   Oid *paramTypes, int numParams)
{
	...
	/*
	 * (3) Apply provenance rewritting (if we use the optimizer to generate plans for
	 * 		all rewrites, we must not rewrite the query here.
	 */
	if (!prov_use_optimizer)
		querytree_list = provenanceRewriteQueryList(querytree_list);

	/* debug for rewritten provenance query trees */
...
pg_plan_query(Query *querytree, int cursorOptions, ParamListInfo boundParams)
{
	...
	if (prov_use_optimizer)
	{
		elog(DEBUG1, "---------------- use optimizer that plans all possible rewrites ");
				...
				bool needSnapshot)
{
	...
	elog(DEBUG1, "---------------- start plan queries");
	...
	elog(DEBUG1, "---------------- finished plan queries");
